...
export class Information_Distance_Point {
    constructor(planet) {
        ...
        this.radius = planet.computeRadius * 3;
        this.init();
    };
    init() {
        this.create();
...
class Line {
    constructor(radius) {
        this.points = [];
        this.range = (radius / 100) * 700;
        this.weight = (radius / 100) * 2;
        this.init();
    };
    init() {
        this.createVert();
    ...
    update(theta, position) {
        this.mesh.rotation.set(0, theta, 0); // オブジェクトの向きを常にカメラの方向
        this.mesh.position.copy(position); // オブジェクトの位置を中心に設定
    };
};



export class Distance_Information {
    constructor(planet) {
        this.base = planet.computeRadius * 0.44;
        this.theta = planet.alphaRad + (90 * Math.PI) / 180;
        this.phi = (0 * Math.PI) / 180;
        this.speed = planet.computeRevolution;
        this.radius = planet.computeRadius * 3;
    ...
    update(planet) {
        ...
        this.line.update(look, position);
    };
};